...
const NavBar: React.FC = () => {
  ...
  const { isChecked, setIsChecked } = useColorScheme();

  const { jwtToken, setJwtToken } = useAuth();
  const navigate = useNavigate();

  // const [isChecked, setIsChecked] = useState<boolean>(false);
  // const [isChecked, setIsChecked] = useColorScheme() as [boolean, Dispatch<SetStateAction<boolean>>];
  const handleCheckboxChange = () => {
    // setIsChecked(!isChecked);
    setIsChecked((prevValue: boolean) => {
      if (prevValue === null) return false; // or true, depending on your logic
      return !prevValue;
    });
  };
  ...
  return (
    <nav className="bg-gray-800 fixed w-full z-10 shadow-lg drop-shadow-2xl">
      <div className="max-w-full px-4 mx-auto sm:px-6 lg:px-8 " >
        {/* <div className="flex items-center justify-between h-16"> */}
        <div className="flex items-center justify-between h-16">
          <div className="flex items-center">
            <Link to="/">
              <h1 className="text-red-600 text-4xl font-bold cursor-pointer mr-8">
                SAFEFLIX
              </h1>
            </Link>
          </div>
              ...
              <div className='shadow-card flex h-[46px] w-[82px] items-center justify-center rounded-md bg-white'>
                <span
                  className={`flex h-9 w-9 items-center justify-center rounded ${
                    isChecked ? 'bg-primary text-black' : 'text-body-color bg-red-600 text-white'
                  }`}
                >
                  <svg
                    width='16'
            ...
            <div className="flex items-center space-x-4">
              {jwtToken ? (
                ...
                <>
                  ...
                  <Link to="/singup">
                    <button className="bg-red-600 px-6 py-2 rounded cursor-pointer text-white hover:bg-red-500 hover:text-white">
                      Sign Up
                    </button>
                  </Link>
                </>
